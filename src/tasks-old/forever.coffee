...
class Forever
	...
	forever: (args) =>
		...
		build = @server.folders.temp.server
		build = @server.folders.temp.index if @server.config.type is 2
	...
	start: ->
		...
		src = @server.folders.temp.server
		src = @server.folders.temp.index if @server.config.type is 2
